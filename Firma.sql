...
--1. Scrie?i ?i testa?i o func?ie care returneaz? angaja?ii a c?ror func?ii con?ine o secven?? de caractere primit? ca parametru.

Select * From ANGAJATI

CREATE FUNCTION dbo.Angajati_functie(@functie VARCHAR(10))
RETURNS TABLE AS
RETURN
    SELECT F.IdFunctie, A.Nume, A.Prenume
		FROM ANGAJATI A  JOIN Functii F ON A.IdFunctie = F.IdFunctie
    WHERE F.Denumire LIKE @functie
        GROUP BY A.Nume, A.Prenume, F.IdFunctie

SELECT * FROM dbo.Angajati_functie('MANAGER')


--2. Scrie?i ?i testa?i o func?ie care returneaz? salariile dintr-un departament primit ca parametru. Scrie?i o interogare care folose?te func?ia pentru a determina câ?i angaja?i beneficiaz? de fiecare salariu.

Select * From ANGAJATI
Select * From DEPARTAMENTE
Select * From FUNCTII

CREATE FUNCTION dbo.Departamente_salariu(@functie VARCHAR(10))
RETURNS TABLE
AS
RETURN
    SELECT D.Denumire, F.Salariu, COUNT(*)Numar_angajati
    FROM ANGAJATI A  
        JOIN DEPARTAMENTE D ON A.IdDept = D.IdDept
        JOIN FUNCTII F ON A.IdFunctie = F.IdFunctie
    WHERE D.Denumire LIKE @functie
        GROUP BY F.Salariu, D.Denumire

SELECT * FROM dbo.Departamente_salariu('PRODUCTIE')


--3. Scrie?i ?i testa?i o func?ie care returneaz? salariul minim ?i maxim dintr-un departament primit ca parametru.

Select * From ANGAJATI
Select * From DEPARTAMENTE
Select * From FUNCTII

CREATE FUNCTION dbo.Salariu_med(@functie VARCHAR(30))
RETURNS TABLE
AS
RETURN
    SELECT  min(F.Salariu) AS Salariu_minim,  max(F.Salariu) As Salariu_maxim
    FROM ANGAJATI A  
        JOIN DEPARTAMENTE D ON A.IdDept = D.IdDept
        JOIN FUNCTII F ON A.IdFunctie = F.IdFunctie
    WHERE D.Denumire LIKE @functie

 SELECT * FROM dbo.Salariu_med('PROIECTARE')


 --4. Scrie?i ?i testa?i o func?ie care returneaz? produsele vândute într-o anumit? perioad? de timp. Limitele perioadei de timp sunt trimise ca parametri c?tre func?ie. 

 SELECT *FROM VANZARI
 SELECT *FROM PRODUSE

 CREATE FUNCTION dbo.Vanzari_produse(@inceput date,@sfarsit date)
RETURNS TABLE
AS
RETURN
    ...
    WHERE  V.DataVanz BETWEEN @inceput AND @sfarsit  --BETWEEN se foloseste pentru a selecta valori care se afl? în interval specificat
	Group by P.Denumire,  V.DataVanz

 SELECT * FROM dbo.Vanzari_produse('2018-07-03','2021-08-06' )


--5. Scrie?i ?i testa?i o func?ie care returneaz? suma total? încasat? de un vânz?tor al c?rui nume este trimis ca parametru. Scrie?i si testa?i o func?ie care se bazeaz? pe prima ?i care verific? dac? suma dep??e?te un anumit prag minim trimis ca parametru. Afi?a?i angaja?ii care au vândut produse în valoare mai mare decât 100 RON


 SELECT *FROM VANZARI

 CREATE FUNCTION dbo.Castig_mediu(@vanzator int)
RETURNS TABLE
AS
RETURN
    SELECT  V.IDVanzator, sum(V.NrProduse*V.PretVanz) AS SUMA
    FROM VANZARI V
		JOIN Produse P ON V.IDProdus = P.IdProdus
    WHERE V.IDVanzator like @vanzator  
	Group by V.IDVanzator
	
 SELECT * FROM dbo.Castig_mediu(14)

CREATE FUNCTION dbo.Verificare_Castig_mediu(@vanzator int, @minim int)
RETURNS TABLE
AS
RETURN
GO
    SELECT  C.IdVanzator, C.SUMA
    FROM dbo.Castig_mediu(@vanzator) AS C
	WHERE C.SUMA > @minim

 SELECT * FROM dbo.Verificare_Castig_mediu(15, 100)


 --6. Scrie?i ?i testa?i o func?ie care returneaz? cele mai vândute N produse, într-o anumit? perioad? de timp. Valoarea lui N ?i limitele perioadei de timp sunt trimise ca parametri c?tre func?ie. 

CREATE FUNCTION dbo.udf_top_produse (@top int, @data_inceput date, @data_sfarsit date) 
RETURNS TABLE 
AS 
RETURN 
SELECT TOP(@top) v.IdProdus, p.Denumire, SUM(NrProduse) AS total_cantitate 
	FROM Vanzari v 
		INNER JOIN Produse p  ON v.idprodus = p.idprodus 
	WHERE V.DataVanz BETWEEN @data_inceput AND @data_sfarsit 
	GROUP BY v.idprodus, p.denumire 

SELECT * FROM dbo.udf_top_produse(6, '2016-06-02', '2021-08-06') 
	

--7. Scrie?i ?i testa?i o func?ie care returneaz? clien?ii ordona?i descresc?tor dup? sumele cheltuite, într-o anumit? perioad? de timp ale c?rei limite sunt trimise ca parametri. 


SELECT *FROM CLIENTI
SELECT *FROM VANZARI

CREATE FUNCTION dbo.Ordine_desc1 (@data_inceput1 date, @data_sfarsit1 date)
RETURNS TABLE
AS
RETURN
    SELECT C.IdClient,C.Denumire, sum(V.NrProduse*V.PretVanz) AS BANI
    FROM VANZARI V
		JOIN CLIENTI C ON V.IDClient = C.IdClient
    WHERE V.DataVanz BETWEEN @data_inceput1 AND @data_sfarsit1 
	Group by C.IdClient, C.Denumire

SELECT *FROM dbo.Ordine_desc1( '2016-06-02', '2021-08-06')
ORDER BY BANI DESC
